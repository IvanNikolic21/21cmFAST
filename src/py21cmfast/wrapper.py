...
def drdz(z, cosmo_params):
    """Comiving distance (in cm) per unit redshift."""
    return (1.0 + z) * constants.c.cgs.value * dtdz(z, cosmo_params)


def dtdz(z, cosmo_params):
    """The value of dt/dz at the redshift parameter z."""
...
class TaueParams:
    """Class for stuff needed in the optical depth calculation."""
...
def _dtau_e_dz(z_en, z, xH, length, cosmo_params):
    """Calculation of optical depth per redshift."""
    p = TaueParams(z, xH, length, cosmo_params)
    if (p.length == 0) or not (p.z):
        return (1 + z_en) * (1 + z_en) * drdz(z_en, cosmo_params)
    else:
        if p.z[0] > z_en:
            return (1 + z_en) * (1 + z_en) * drdz(z_en, cosmo_params)
        ...
        xH_inter = p.xH[i - 1] + (p.xH[i] - p.xH[i - 1]) / (p.z[i] - p.z[i - 1]) * (
            z_en - p.z[i - 1]
        )
        xi = 1.0 - xH_inter
        if xi < 0:
        ...
        if xi > 1:
            xi = 1
        return xi * (1 + z_en) * (1 + z_en) * drdz(z_en, cosmo_params)


def _tau_e(
    zstart, zend, z_array, x_array, length, cosmo_params, kSZ_consts
):  # integration of the previous function to get the optical depth

    if zend > kSZ_consts.Zreion_HeII and zstart < kSZ_consts.Zreion_HeII:
        prehelium, _ = quad(
            _dtau_e_dz,
            kSZ_consts.Zreion_HeII,
            zstart,
            args=(z_array, x_array, length, cosmo_params),
        )
        posthelium, _ = quad(
            _dtau_e_dz,
            zend,
    ...
    elif zend > kSZ_consts.Zreion_HeII and zstart >= kSZ_consts.Zreion_HeII:
        prehelium = 0
        posthelium, _ = quad(
            _dtau_e_dz, zend, zstart, args=(z_array, x_array, length, cosmo_params)
        )
    else:
        posthelium = 0
        prehelium, _ = quad(
    ...
    return constants.sigma_T.cgs.value * (
        (kSZ_consts.N_b0 + kSZ_consts.He_No) * prehelium + kSZ_consts.N_b0 * posthelium
    )
